    ...
    <div className="flex flex-col gap-4 p-5 sm:p-6">
      {config.sentences.map((sentence) => {
        ...
        return (
          <div
            key={sentence.id}
            className="flex flex-wrap items-center gap-2 rounded-xl border-2 border-activity/20 bg-activity-soft/40 p-3.5 text-sm"
          >
            <span>{before}</span>
            <Input
              ...
              disabled={submitted}
              className={`inline-block w-40 bg-background ${
                submitted ? (isCorrect ? "border-emerald-600" : "border-destructive") : "border-activity/40"
              }`}
            />
            <span>{after}</span>
            {submitted && !isCorrect && (
